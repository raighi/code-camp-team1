...
def main():
    ...
    try:
        ...
        if options.command == 'create': #Cas de la commande create
            commands.create(new_file=options.new_file, details=' '.join(options.new_details), user=' '.join(options.user)) #Appel de la fonction create et attibution des paramètres récupérés par le parser
